...
-- SECURITY: Only 'consumer' and 'company' roles are allowed from registration.
-- Admin accounts can only be created directly in the database by an existing admin.
...
DECLARE
  ...
  safe_role user_role;
BEGIN
  requested_role := COALESCE(NEW.raw_user_meta_data->>'role', 'consumer');
  
  IF requested_role IN ('consumer', 'company') THEN
    safe_role := requested_role::user_role;
  ELSE
    safe_role := 'consumer'::user_role;
  END IF;
